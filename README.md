...
- **Secrets**: `nullafi_license_key`, `nullafi_cypher_key`, `proxy_mitm_key`, and `acme_dns01_env` are marked `sensitive`; the license key, Shield ICAP cypher key, and Squid MITM cert/key are stored in AWS Secrets Manager, not passed to containers as plain environment values.
- **Shield ICAP cypher/tokenization**: `nullafi_cypher_token_prefix` (default `"NFA_"`) and `nullafi_cypher_algorithm` (default `"aes-256-siv"`) are always set as plain environment values on the shield-icap container (`NULLAFI_CYPHER_TOKEN_PREFIX`, `NULLAFI_CYPHER_ALGORITHM`). `nullafi_cypher_key` (`NULLAFI_CYPHER_KEY`) is optional and has no default; when set, it's stored in AWS Secrets Manager (like `nullafi_license_key`) and passed to shield-icap only, at runtime, via `secrets`/`valueFrom` — never as a plain environment value. When unset, no secret is created and the env var is omitted entirely.
- **Client IP on the Squid proxy**: the `squid` target group has `proxy_protocol_v2 = true` ([AWS guide](https://aws.amazon.com/blogs/networking-and-content-delivery/preserving-client-ip-address-with-proxy-protocol-v2-and-network-load-balancer/)), and the Squid container gets `USING_LOAD_BALANCER=yes`, which makes squid.conf require a PROXY v2 header on its listening port (`require-proxy-header`) — this is how Squid (and, downstream, the ICAP `X-Client-IP` header) learns the real client IP instead of the NLB's. `proxy_protocol_v2` is a target-group attribute, not a load-balancer/listener one, so enabling it only on the `squid` target group has no effect on `shield_web_http`/`shield_web_https`, which share the same NLB — no separate load balancer is needed for this.
...
| <a name="input_nullafi_cypher_algorithm"></a> [nullafi\_cypher\_algorithm](#input\_nullafi\_cypher\_algorithm) | Cypher algorithm for Shield ICAP tokenization (env value for NULLAFI\_CYPHER\_ALGORITHM). | `string` | `"aes-256-siv"` | no |
| <a name="input_nullafi_cypher_key"></a> [nullafi\_cypher\_key](#input\_nullafi\_cypher\_key) | Nullafi cypher key for Shield ICAP (env value for NULLAFI\_CYPHER\_KEY). Optional; when set, stored in AWS Secrets Manager and passed to the shield-icap container at runtime via secrets (valueFrom) rather than as a plain environment value. Omitted entirely when unset. | `string` | `null` | no |
| <a name="input_nullafi_cypher_token_prefix"></a> [nullafi\_cypher\_token\_prefix](#input\_nullafi\_cypher\_token\_prefix) | Token prefix for Shield ICAP tokenization (env value for NULLAFI\_CYPHER\_TOKEN\_PREFIX). | `string` | `"NFA_"` | no |
