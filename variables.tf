# ------------------------------------------------------------------------------
# This module does not configure an AWS provider — the caller does. Authenticate
# via the standard AWS credential chain (env vars, shared config/profile, or an
# IAM role) rather than passing access keys through variables.
#
# NOTE: No ALB-related variables – this module uses service discovery only.
# ------------------------------------------------------------------------------

variable "region" {
  description = "AWS region. Must match the region the caller's AWS provider is configured for (used to build ARNs and VPC endpoint service names)."
  type        = string
}
